...
**Reference:** the sibling review [`../../prototype-simple/docs/RESPONSIVE-REVIEW.md`](../../prototype-simple/docs/RESPONSIVE-REVIEW.md) — same method, same breakpoints, same patterns, so both prototypes stay one design system
...
sheets and full-screen sheets on iOS and Android, and the simple prototype's review for consistency.
...
| M8 | Low | `#logo-title` | No app title on small phones (the logo was hidden below 480 px, the first header row was empty). | Fixed: short title "BBL Liegenschaften Inventar" on phones and tablets (simple prototype: title in the menu header; here the header has room for it). |
...
| B3 | Medium | `resetFilters()` | Selected `#filter-pane input` — an element that does not exist (the drawer is `#smart-drawer`). **Reset did not uncheck the boxes**; desktop too. The same bug was found in the simple prototype (its B1). | Fixed. |
...
| B5 | Low | Tokens | `var(--primary)` (undefined) on the Geokatalog checkbox accent, the legend border and legend links; `var(--grey-400)` (undefined) on the toast close button. | Fixed: `--primary-red` / `--interactive-blue`; `--grey-400: #A0A8AE` added as in `prototype-simple/css/tokens.css`. |
...
| Phase | Change | Notes |
|---|---|---|
| 1 Library | `vendor/maplibre-gl/` (MapLibre GL JS 5.19, BSD-3) copied from `prototype-simple`; the Mapbox CDN tags and the access token are gone. | Same build in both prototypes; the site works offline from the repo except for tiles. |
| 2 Basemaps | The four Mapbox styles became the simple prototype's set: CARTO Positron ("Light", default), CARTO Voyager ("Standard"), swisstopo SWISSIMAGE raster ("Luftbild"), CARTO Dark Matter ("Dark"). `mapStyles` mirrors `prototype-simple/js/config.js`. | Mapbox "Hybrid" (aerial + streets) has no free equivalent. A saved `mapStyle` with an old Mapbox id falls back to Light. |
| 3 Thumbnails | Four local PNGs in `assets/basemaps/`, rendered from the styles themselves (160 × 120 @2x). | The Mapbox Static Images API needed the token; CARTO's raster tiles are watermarked "API KEY REQUIRED" (the simple prototype still uses them — its O5). |
...
**Known differences.** No globe at low zoom (MapLibre renders Mercator by default, like the simple prototype); "Standard"
is OpenMapTiles cartography instead of Mapbox Streets; "Luftbild" covers Switzerland only (swisstopo, zoom ≥ 8) — the
foreign objects of the portfolio sit on an empty background in that style, exactly as in the simple prototype (O13).

## 4. Recommendations (not implemented)

| ID | Recommendation |
|---|---|
| O1 | **Table on phones.** The list view is a horizontally scrolling table with four columns; a card list (one card per row, sort menu, export in the menu) is the phone pattern — same as O1 of the simple prototype. |
...
| O6 | **Tablet portrait with the drawer open** leaves a 428 px map. Below ~900 px the drawer could overlay the map instead of pushing it (simple prototype O9). |
| O7 | **Legacy CSS.** Remove the `#filter-pane` block (B6); rename `.btn-back` where it styles the "Hinzufügen" buttons. |
| O8 | **Add the harness to the repo** (`test/responsive/`, `playwright-core` as dev dependency) so the numbers in §5 are reproducible in CI — the same recommendation as the simple prototype's O10; one harness could serve both. |
...
| O10 | **Language selector and account menu** from the wireframe ("DE ▾", "DM") need i18n data and a user model that this prototype does not have; the simple prototype has both. |
...
| `vendor/maplibre-gl/` | MapLibre GL JS 5.19 + CSS + licence, copied from `prototype-simple`. |
