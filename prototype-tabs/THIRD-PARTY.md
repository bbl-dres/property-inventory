...
Everything in this prototype that was not written for it: the vendored library, the bundled assets and
...
| Topic images and names | [geoadmin/web-mapviewer](https://github.com/geoadmin/web-mapviewer): the `topics.png` sprite and the German topic names of its locale file (`TOPIC_LABELS` in `js/app.js`) | BSD-3-Clause, © 2022 swisstopo | `assets/topics.png`, `assets/LICENSE-web-mapviewer.md` | "Thema wechseln" topic grid |
...
| swisstopo WMTS (`wmts.geo.admin.ch`) | SWISSIMAGE aerial basemap and Geokatalog layers that offer WMTS tiles | © swisstopo |
| swisstopo WMS (`wms.geo.admin.ch`) | Rendering of the other Geokatalog layers added to the map | © swisstopo |
| geoadmin API (`api3.geo.admin.ch`) | Location and layer search (SearchServer), topics and catalog trees (CatalogServer), layer metadata and legends (MapServer), feature identification (identify) | No key required |
| Unsplash (`images.unsplash.com`) | Placeholder photos of the detail carousel (`placeholderImages` in `js/app.js`) | [Unsplash License](https://unsplash.com/license); placeholders only |
