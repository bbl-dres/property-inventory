...
module.exports = {
  name: 'tabs: partial data (parcels, land cover and contracts missing)',
  boot: {
    prototype: 'prototype-tabs',
    url: 'http://localhost/prototype-tabs/?view=detail&id=1080%2F4840%2FAF&tab=contracts',
    fetch: { 'data/parcels.geojson': { status: 404 }, 'data/landcovers.geojson': { status: 404 }, 'data/contracts.json': { status: 404 } }
  },
  allowedErrors: [],
  async run(ctx, check) {
    ...
    check('parcels and land cover null, contracts empty', state.parcelData === null && state.landCoverData === null && state.allContracts.length === 0);
    check('warning toast shown', !!document.querySelector('#toast-container .toast-warning'));
    check('building layers present, parcel and land cover layers absent', !!map.getLayer('buildings-points') && !map.getLayer('parcels-fill') && !map.getLayer('landcovers-fill'));
    check('parcel and land cover tables empty without data', document.querySelectorAll('#parcels-body tr[data-parcel-id]').length === 0 && document.querySelectorAll('#landcovers-body tr[data-landcover-id]').length === 0);
