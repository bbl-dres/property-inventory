// Reports drift between the common modules of prototype-main and prototype-tabs.
...
const COMMON_JS = ['utils', 'i18n', 'toast', 'geo', 'keys', 'boot', 'basemaps', 'map-controls', 'measure',
  'context-menu', 'swisstopo', 'print', 'table', 'carousel', 'mini-map', 'gestures', 'accordion'];
const COMMON_FILES = COMMON_JS.map(n => 'js/' + n + '.js').concat(['data/i18n.json']);
...
for (const rel of COMMON_FILES) {
  const a = path.join(ROOT, 'prototype-main', rel);
...
for (const n of ALIGNED_JS) {
  const a = path.join(ROOT, 'prototype-main', 'js', n + '.js');
