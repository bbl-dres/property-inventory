# Code Review — prototype-simple

**Date:** 2026-09-11
**Scope:** all files under `prototype-simple/` (16 ES modules, `index.html`, `styles.css`, `data/i18n.json`)
...
| R9 | The jsdom test harness written for this review (fake MapLibre, three scenarios, ~60 checks) lives outside the repository. Adding it under `prototype-simple/test/` with `jsdom` as a dev dependency would give the project a regression suite; it needs a `package.json`, which the "no build step" setup currently avoids. |
