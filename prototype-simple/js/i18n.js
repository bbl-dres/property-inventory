// i18n: lightweight internationalisation runtime shared by the prototypes.
// Loads one JSON file with all languages ({ "key": { "de": "...", "en": "..." } }) and
// exposes t() for keyed lookups. German is the default; ?lang= selects another language.

let allTranslations = null; // cached: { de: { key: value }, en: { key: value }, ... }
let loadFailed = false;     // true when the translation file could not be loaded (UI shows raw keys)
let translations = {};
let currentLang = 'de';
let persistDefault = true;  // write ?lang=de into the URL even when no language was requested
...
// t() with a static fallback for the case that the translation file failed to load
...
// Locale string for Intl formatters (e.g. 'de-CH', 'fr-CH')
...
//   url:     path of the translation file (relative to the page)
//   options: { persistLang: false } keeps the URL untouched unless a language was requested
export function initI18n(url, options) {
  options = options || {};
  if (options.persistLang === false) persistDefault = false;
  const lang = detectLang();
  currentLang = lang;
  return loadAllTranslations(url).then(function() {
    translations = (allTranslations && allTranslations[lang]) || {};
    applyTranslationsToDOM();
    document.documentElement.lang = lang;
...
function persistLang(lang, explicit) {
  const url = new URL(window.location);
  if (!explicit && !persistDefault && !url.searchParams.has('lang')) return;
...
function loadAllTranslations(url) {
...
export function applyTranslationsToDOM(root) {
  (root || document).querySelectorAll('[data-i18n],[data-i18n-placeholder],[data-i18n-title],[data-i18n-aria-label],[data-i18n-alt]').forEach(function(el) {
