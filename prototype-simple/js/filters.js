...
// Filters: URL state, filter drawer, option lists, pills and the map filter.
...
export function getFiltersFromURL() {
  ...
  const filters = readFilterParams(params, Object.keys(filterConfig));
  // Preserve older links that used the previous demo status label.
  filters.status = filters.status.map(value => value === 'Aktiv' ? 'In Betrieb' : value);
  return filters;
}
...
export function navigateWithLandFilter() {
  if (state.currentDetailBuilding) navigateWithFilter('land', state.currentDetailBuilding.properties.adr_land);
}

export function navigateWithOrtFilter() {
  if (state.currentDetailBuilding) navigateWithFilter('ort', state.currentDetailBuilding.properties.adr_ort);
}
