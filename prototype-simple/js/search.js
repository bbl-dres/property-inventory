// Header search: local objects, swisstopo locations and Geokatalog layers, with a scope menu
...
function getLocalSearchText(feature) {
  ...
  if (text === undefined) {
    ...
    text = [p.bbl_bez, p.adr_conct, p.adr_ort, p.bbl_id].map(function(v) { return v == null ? '' : String(v); }).join(' ').toLowerCase();
...
export function initSearch() {
  ...
  // History items: remove button and re-search
  ...
  function performSearch(term) {
    ...
    const local = { type: 'local', data: [] };
    if (scopeAllows('objects') && state.buildingsData) {
      local.data = state.buildingsData.features.filter(function(f) { return getLocalSearchText(f).indexOf(lowerTerm) !== -1; });
    }
  ...
  function renderSearchResults(results, term) {
    ...
    // Objekte (local): icon, title + address, meta. Property values are escaped; the term is highlighted.
